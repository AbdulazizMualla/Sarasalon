 ...
 class Upload {
   ...
   public function __construct($uploadDir , $rootDir = false)
   {
     ...
     if($rootDir){
       ...
       $this->rootDir = $_SERVER['DOCUMENT_ROOT'].DIRECTORY_SEPARATOR.'SaraSalon/';
   ...
   protected function createUploadDir(){
     echo "<script>console.log('$this->uploadDir')</script>";
   ...
   protected function isMimeAllowed(){
     ...
     $fileMimType = mime_content_type($this->file['tmp_name']);

     if (!in_array($fileMimType,$allowed)) {
       return false;
     }
     return true;
   }
